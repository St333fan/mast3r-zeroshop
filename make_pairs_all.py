...
def process_directories(base_path, backbone=None, retrieval_model=None):
    """Process both surface and segmented directories with different scene graphs"""
    # Define subdirectories and their corresponding scene graphs
    subdir_configs = {
        'surface': 'retrieval-20-2',
        'segmented': 'retrieval-30-2'
    }
    
    for subdir, scene_graph in subdir_configs.items():
        images_dir = path.join(base_path, 'train_pbr', 'mast3r-sfm', subdir, 'images')
        output_file = path.join(images_dir, 'pairs.txt')
        
        if not path.exists(images_dir):
            print(f"Warning: Directory {images_dir} does not exist, skipping...")
            continue
            
        print(f"Processing {subdir} directory: {images_dir} with scene_graph: {scene_graph}")
        main(images_dir, scene_graph, output_file, backbone, retrieval_model)
...
if __name__ == '__main__':
    ...
    # Always use retrieval since both directories need it
    ...
    process_directories(args.path, backbone, retrieval_model)
